...
const sessions = [
  ...
  { id: "monza-2024", year: 2024, match: { country_name: "Italy" } },
...
/** Telemetry along one lap, resampled every `step` metres. */
function resample(car, loc, startMs, step = 8) {
  // Distance by integrating speed over time (km/h → m/s).
  const pts = car
  ...
  const at = (arr, key, t) => {
    let i = arr.findIndex((p) => p[key] >= t);
    if (i <= 0) return i === 0 ? { a: arr[0], b: arr[0], k: 0 } : { a: arr.at(-1), b: arr.at(-1), k: 0 };
    const a = arr[i - 1];
    const b = arr[i];
    return { a, b, k: (t - a[key]) / (b[key] - a[key] || 1) };
  };
  const out = [];
  for (let s = 0; s <= d; s += step) {
    const { a, b, k } = at(pts, "d", s);
  ...
  return { length: Math.round(d), samples: out };
}
...
for (const s of sessions) {
  ...
  for (const d of drivers) {
    ...
    const trace = resample(car, loc, start);
