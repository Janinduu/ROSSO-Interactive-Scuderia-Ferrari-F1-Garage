...
function rawAnchors(spec: CarSpec): Partial<Record<PartId, Anchor>> {
  const a: Partial<Record<PartId, Anchor>> = {
    tyres: [
      spec.frontAxle,
      spec.tyreF.r + 0.05,
      spec.trackF + spec.tyreF.w / 2,
    ],
    brakes: [
      spec.rearAxle,
      spec.tyreR.r,
      spec.trackR - spec.tyreR.w / 2 - 0.06,
    ],
    steering: [spec.cockpit.x0 + 0.1, spec.cockpit.y + 0.06, 0],
  };
  const engineStart = spec.engineFront
    ? spec.frontAxle - 0.15
    : spec.cockpit.x1 + 0.22;
  const engineLength = spec.engineFront
    ? 0.95
    : Math.min(0.95, spec.rearAxle - engineStart - 0.12);
  a["power-unit"] = [
    engineStart + engineLength / 2,
    spec.family === "wing70s" ? 0.53 : 0.6,
    0,
  ];
  if (spec.frontWing)
    a["front-wing"] = [
      spec.frontWing.x,
      spec.frontWing.y + 0.06,
      spec.frontWing.span * 0.36,
    ];
  if (spec.rearWing)
    a["rear-wing"] = [
      spec.rearWing.x,
      spec.rearWing.y + 0.05,
      spec.rearWing.span * 0.3,
    ];
  if (spec.floor)
    a.floor = [(spec.floor.x0 + spec.floor.x1) / 2, 0.08, spec.floor.w + 0.02];
  if (spec.diffuser)
    a.diffuser = [
      spec.diffuser.x1 - 0.12,
      0.08 + spec.diffuser.rise * 0.8,
      spec.diffuser.w * 0.6,
    ];
  if (spec.pods)
    a.sidepods = [
      spec.pods.x0 + 0.35,
      spec.pods.y + spec.pods.h * 0.8,
      spec.pods.z + spec.pods.w * 0.7,
    ];
  return a;
}
